(function($R)
{
    $R.add('plugin', 'imageselector', {
        ...
        modals: {
            ...
            + '</span>'
            + '</div>'
            + '<div id="imageselector-container" class="raw-block-400 siravel-row raw-margin-top-24" style="overflow: scroll;">Loading your image collection...</div>'
            + '</section>'
        },
        open: function () {
            var options = {
                ...
                success: $.proxy(function(data)
                {
                    ...
                    if (data.data.length > 0) {
                        $.each((data.data), $.proxy(function(key, val)
                        {
                            ...
                            var img = $('<div class="raw25 float-left thumbnail-box"><div class="img siravel-thumbnail-img" style="background-image: url(\'' + val.js_url + '\')" data-img-name="'+ val.js_url +'" src="' + val.js_url + '" rel="' + val.js_url + '" title="' + thumbtitle + '"></div></div>');
                    ...
                    $("#imageselector-filter").bind("keyup", function(){
                        $(".siravel-thumbnail-img").each(function(){
