...
class TempleiroCallbacks
{
    ...
    public function handle($event, $payload)
    {
        ...
        // Ignore
        if (!app()->bound(TempleiroService::class)) {
            return true;
        }
        ...
        if ($method == 'onBooting') {
            $model::addGlobalScope(new TempleiroScope);
            return true;
        }

        if ($method == 'onCreating') {
            $business = app()->make(TempleiroService::class);
