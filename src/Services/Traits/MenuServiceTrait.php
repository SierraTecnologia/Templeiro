...
use Templeiro\Repositories\LinkRepository;
use Templeiro\Repositories\MenuRepository;
use Templeiro\Repositories\PageRepository;
...
trait MenuServiceTrait
{
    ...
    public function menu($slug, $view = null, $class = '')
    {
        ...
        if (Gate::allows('cms', Auth::user())) {
            ...
            $response .= '<a href="'.url(config('siravel.backend-route-prefix', 'cms').'/menus/'.$menu->id.'/edit').'" class="btn btn-sm ml-2 btn-outline-secondary"><span class="fa fa-edit"></span> Edit</a>';
        }
