...
@section('content')
    ...
    @include('features.events.breadcrumbs', ['location' => ['edit']])
    ...
    <div class="row raw-margin-bottom-24">
        <ul class="nav nav-tabs">
            @foreach(config('siravel.languages', Siravel::config('siravel.languages')) as $short => $language)
                <li role="presentation" @if (request('lang') == $short || is_null(request('lang')) && $short == Siravel::config('siravel.default-language'))) class="active" @endif><a href="{{ url('admin/events/'.$event->id.'/edit?lang='.$short) }}">{{ ucfirst($language) }}</a></li>
            @endforeach
        </ul>
    </div>
