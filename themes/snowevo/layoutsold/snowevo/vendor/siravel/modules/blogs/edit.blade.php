...
@section('content')
    ...
    @include('features.blogs.breadcrumbs', ['location' => ['edit']])
    ...
    <div class="row raw-margin-bottom-24">
        <ul class="nav nav-tabs">
            @foreach(config('siravel.languages', Siravel::config('siravel.languages')) as $short => $language)
                <li role="presentation" @if (request('lang') == $short || is_null(request('lang')) && $short == Siravel::config('siravel.default-language'))) class="active" @endif><a href="{{ url('admin/blog/'.$blog->id.'/edit?lang='.$short) }}">{{ ucfirst($language) }}</a></li>
            @endforeach
        </ul>
    </div>
