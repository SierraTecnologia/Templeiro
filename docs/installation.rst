...
All users using Templeiro will be identified as the same user on the system.
Every user will have the same push URL (``ssh://user@example.org/foobar.git``).
In the application, Templeiro accepts or deny access to the user, depending of
his credentials.
...
Go to `download page <http://templeiro.com/download>`_ and download latest
version from website.

Uncompress it to your prefered location, let's assume */var/www/templeiro*.
...
.. code-block::
    ...
    <VirtualHost *:80>
        ...
        DocumentRoot /var/www/templeiro/web
    </VirtualHost>
...
This step-by-step configuration will guide you through the setup process of Templeiro.
...
Templeiro needs to delegate jobs to the background. For this reason, Templeiro needs to run
...
.. code-block:: shell
    ...
    $ ./app/console templeiro:process-jobs
